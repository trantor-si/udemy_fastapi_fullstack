...
from services.users import get_access_token, get_login_response, \
    get_logout_response, get_password_hash, get_register_response, \
    register_user, LoginForm
...
async def login(request: Request, db: Session = Depends(get_session)):
    try:
        ...
        if not validate_user_cookie:
            ...
            return get_login_response({"request": request, "msg": "Incorrect Username or Password"})
        else:
            return response
...
async def register(request: Request):
    ...
    return get_register_response({"request": request})
...
async def register_user(request: Request, 
    ...
    # validation1 = db.query(UsersModel).filter(UsersModel.username == username).first()
    # validation2 = db.query(UsersModel).filter(UsersModel.email == email).first()
